...
SCENES = [
    ...
    # 14: 설계 장면 6 과 **배치·초기조건 동일, 유속만 0.10**.
    #   PID 의 실측 성공 경계가 0.10 이므로, PID 컷을 이 조건에서 찍는다.
    #   ※ RL 컷(유속 0.18)과 나란히 두면 **서로 다른 문제**를 푼 것이다.
    #      최종 발표용 비교는 반드시 같은 유속으로 다시 낼 것.
    dict(berth_x=0.0, cur=+0.10, d0=9.5, bear=-5.0, yaw_err=+10.0,
         gear=[(0.0, 4.2, 0.95, 0.70, 20.0, 0.10),
               (-2.4, 6.2, 0.85, 0.65, 110.0, 0.13)]),
]
