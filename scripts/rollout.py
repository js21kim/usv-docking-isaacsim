...
parser.add_argument("--scene", type=int, default=0, help="고정 장면 번호 (0~4)")
...
def main():
    ...
    for t in range(args_cli.max_steps):
        ...
        lid = G.lidar_scan(pos, yaw, env._scene_p, n_bins=s.lidar_det_h_bins)
        fls = G.fls_scan(pos, yaw, env._scene_p)
        dx, dw, dok = PC.detect_berth(pos, yaw, env._scene_p)
        ...
        with torch.inference_mode():
            if kind == "rl":
                ...
                tgt = torch.stack(
                    [torch.where(dok, pos[:, 0] + dx, pos[:, 0]),
                     torch.where(dok, pos[:, 1] + dw - 0.50 - BB.LOA / 2, pos[:, 1]),
                     torch.full_like(dx, math.pi / 2)], dim=-1)
                act = pid(pos, yaw, env._nu, tgt)
